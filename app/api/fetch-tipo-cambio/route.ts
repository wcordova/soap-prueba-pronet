...
export async function POST(request: Request) {
  const { fechainit }: { fechainit: string } = await request.json();
  if (!fechainit) {
  ...
  try {
    ...
    const vars: TipoCambioItem[] = tipoCambioResult.TipoCambioFechaInicialResult.Vars.Var;

    const solicitud = await prisma.solicitud.create({
      data: {},
    });

    const tipoCambios: TipoCambioData[] = vars.map((item) => {
